...
const app = await withTimeout(window.SellerProApp.ready, 12000, 'Tempo excedido ao iniciar o perfil.');
...
async function fetchMetricSource(source, queryFactory) {
  const result = await withTimeout(
    queryFactory(),
    5000,
    `Tempo excedido ao consultar ${source}.`
  );
  if (result?.error) throw Object.assign(new Error(result.error.message || `Falha ao consultar ${source}.`), { source });
  return { source, rows: result?.data || [] };
}

async function loadOperationalStats() {
  const name = String(profile.nome || '').trim();
  const supportId = String(profile.support_id || '').trim();

  const tasksQuery = () => {
    let query = client
      .from('sp_tasks')
      .select('id,status,owner,updates,created_at,delivery_completed_at,sale_finalized_at');
    return name ? query.ilike('owner', name) : query.limit(0);
  };

  const storesQuery = () => {
    let query = client
      .from('sp_lojas_entregues')
      .select('id,task_id,status_seller_cont,responsavel,created_at,updated_at,delivery_completed_at,sale_finalized_at');
    return name ? query.ilike('responsavel', name) : query.limit(0);
  };

  const sellersQuery = () => {
    let query = client
      .from('sellers')
      .select('id,status,situacao,suporte,suporte_id,data_inicio');
    if (supportId) return query.eq('suporte_id', supportId);
    return name ? query.ilike('suporte', name) : query.limit(0);
  };

  const settled = await Promise.allSettled([
    fetchMetricSource('sp_tasks', tasksQuery),
    fetchMetricSource('sp_lojas_entregues', storesQuery),
    fetchMetricSource('sellers', sellersQuery)
  ]);

  const sources = ['sp_tasks', 'sp_lojas_entregues', 'sellers'];
  const bySource = {};
  const unavailableTables = [];

  settled.forEach((item, index) => {
    const source = sources[index];
    if (item.status === 'fulfilled') bySource[source] = item.value.rows;
    else {
      console.warn(`[Perfil] Fonte de indicador indisponível: ${source}`, item.reason);
      bySource[source] = [];
      unavailableTables.push(source);
    }
  });

  const tasks = bySource.sp_tasks;
  const stores = bySource.sp_lojas_entregues;
  const sellersRows = bySource.sellers;
  const taskStatus = task => normalizeStatus(task.status);

  const deliveredTaskIds = new Set(
    tasks
      .filter(task => taskStatus(task).includes('loja entregue') || taskStatus(task).includes('venda finalizada') || Boolean(task.delivery_completed_at))
      .map(task => String(task.id))
  );
  for (const store of stores) deliveredTaskIds.add(String(store.task_id || store.id));

  const finalizedSales = Math.max(
    tasks.filter(task => taskStatus(task).includes('venda finalizada') || Boolean(task.sale_finalized_at)).length,
    stores.filter(store => normalizeStatus(store.status_seller_cont).includes('venda finalizada') || Boolean(store.sale_finalized_at)).length
  );
  ...
  const activeSellers = sellersRows.filter(seller => {
  ...
  return {
    gamification: calculateGamification({
...
function setMetricsState(state, text, retry = false) {
  const container = document.querySelector('#profileMetricsState');
  const label = document.querySelector('#profileMetricsStateText');
  const button = document.querySelector('#profileMetricsRetry');
...
async function refreshMetrics() {
  setMetricsState('loading', 'Atualizando indicadores...', false);
  try {
    const stats = await loadOperationalStats();
    renderGamification(stats);
    if (stats.unavailableTables.length === 0) {
      setMetricsState('success', 'Indicadores atualizados', false);
    } else if (stats.unavailableTables.length < 3) {
      setMetricsState('partial', 'Dados parciais', true);
    } else {
      setMetricsState('error', 'Indicadores indisponíveis', true);
    }
  } catch (error) {
    console.error('[Perfil] Falha ao atualizar indicadores.', error);
    renderGamification({ gamification: calculateGamification({}), unavailableTables: ['sp_tasks', 'sp_lojas_entregues', 'sellers'] });
    setMetricsState('error', 'Falha ao atualizar indicadores', true);
  }
}
...
function renderGamification(result) {
  ...
  if (unavailableTables.length) {
    $('#profileDataNote').innerHTML = `<span aria-hidden="true">ⓘ</span><span><strong>Dados parciais:</strong> ${unavailableTables.join(', ')} não respondeu dentro do prazo. Os demais indicadores continuam disponíveis.</span>`;
  } else {
    $('#profileDataNote').innerHTML = '<span aria-hidden="true">ⓘ</span><span><strong>Indicadores atualizados:</strong> todas as fontes operacionais responderam normalmente.</span>';
  }
}
...
async function initialize() {
  ...
  // O perfil principal nunca fica bloqueado esperando indicadores operacionais.
...
document.querySelector('#profileMetricsRetry')?.addEventListener('click', () => { void refreshMetrics(); });
