(function bootstrapSellerPro(global) {
  ...
  async function loadProfile(user) {
    ...
    if (storedAvatarPath) {
      const { data, error } = await withTimeout(
        client.storage.from('avatars').createSignedUrl(storedAvatarPath, 60 * 60 * 12),
        5000,
        'Tempo excedido ao carregar a foto do perfil.'
      ).catch(() => ({ data: null, error: new Error('Avatar indisponível') }));
